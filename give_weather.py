...
api_key = '74747f8476a8b60db53dc1a78368fe50'
    ...
    def get_weather(self, event=None):
        ...
        if len(city) > 2:
            ...
            try:
                ...
                messagebox.showerror('Weathy', 'No such city in database')
            except:
                messagebox.showerror('Weathy', 'No internet Connection')
...
if __name__ == '__main__':
    ...
    if not api_key:
        root.withdraw()
        messagebox.showerror('Weathy', 'OpenWeatherMap Api Key is required\n to use this App')
        sys.exit(0)

    bg_list = [f'C:/Users/masee/Weather_Python/backgrounds/bg{i}.png' for i in range(1, 7)]
    bg = Image.open(random.choice(bg_list))
    bg_image = ImageTk.PhotoImage(bg)

    search_icon = PhotoImage(file='search.png')

    clear_weather_icon = PhotoImage(file='clear.png')
    clouds = Image.open('clouds.png')
    cloudy_icon = ImageTk.PhotoImage(clouds)
    high_temp_icon = PhotoImage(file='high_temp.png')
    low_temp_icon = PhotoImage(file='low_temp.png')
    humidity_icon = PhotoImage(file='humidity.png')
    pressure_icon = PhotoImage(file='pressure.png')
    wind_icon = PhotoImage(file='wind.png')
    thunderstorm_icon = PhotoImage(file='thunderstorm.png')
    snow_icon = PhotoImage(file='snow.png')
    drizzle_icon = PhotoImage(file='drizzle.png')
    mist_icon = PhotoImage(file='mist.png')
    hail_icon = PhotoImage(file='hail.png')
